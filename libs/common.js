...
function averageAngles(angles) {
	//angles = angles.map((a)=>{return a*(Math.PI/180)})
	var finalVector = v()
	for (let i = 0; i < angles.length; i++) {
		const a = angles[i];
		finalVector.x += Math.cos(a)
		finalVector.y += Math.sin(a)
	}
	var angle = Math.atan2(finalVector.y,finalVector.x),
		dst = getDst(v(),finalVector)
	return dst<0.001?0:angle
}
...
function polygonPoint(polygon, point) {
	var point = point,
		bounds = getPolygonBounds(polygon),
		outsidePoint = v(bounds.max.x+100, 0)

	var lines = new Array()
	for (let i = 0; i < polygon.length; i++) {
		const p = polygon[i];
		var p2 = polygon[stopOverflow(i+1, polygon.length)]
		lines.push([p,p2])
	}

	var testLine = [point, outsidePoint],
		collisions = 0
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (intersects(
			...
			testLine[0].x,testLine[0].y,testLine[1].x,testLine[1].y,
			)) {
				collisions++
			}
	}
...
function linePoint(point, line) {
    ...
    var lineAngle = (-getAngle(line.p1, line.p2))*(Math.PI/180),


        newLine = {p1:rotate(0,0, line.p1.x, line.p1.y, lineAngle), p2:rotate(0,0, line.p2.x, line.p2.y, lineAngle)},
		newPoint = rotate(0,0, point.x, point.y, lineAngle)
		
    var point = straightLinePoint(newPoint, newLine)

        rotatedPoint = rotate(0,0, point.x, point.y, -lineAngle)
